...
const PropertyTypeSelect: FC<PropertyTypeSelectProps> = ({
  ...
  const { releaseType, setReleaseType } = useSearch();

  const typeOfProperty = [
    {
      name: "Pre-order",
      description: "Albums available for pre-order",
      checked: releaseType.includes("Pre-order"),
    },
    {
      name: "New Release",
      description: "Recently added albums",
      checked: releaseType.includes("New Release"),
    },
    {
      name: "Almost Funded",
      description: "Close to reaching pressing goal",
      checked: releaseType.includes("Almost Funded"),
    },
  ];

  const handleCheckboxChange = (name: string, checked: boolean) => {
    if (checked) {
      setReleaseType([...releaseType, name]);
    } else {
      setReleaseType(releaseType.filter(type => type !== name));
    }
  };
        ...
        <>
          ...
          >
            ...
            <div className="flex-grow">
              <span className="block xl:text-lg font-semibold">Release type</span>
                  ...
                  {typeOfProperty.map((item) => (
                    <div key={item.name} className="">
                      <Checkbox
                        ...
                        onChange={(checked) => handleCheckboxChange(item.name, checked)}
                      />
                    </div>
                  ))}
                </div>
