...
import { DEFAULT_FEATURED_IMAGE } from "@/constants/images";
...
const CardLarge1: FC<CardLarge1Props> = ({
  ...
  onClickNext = () => {},
  onClickPrev = () => {},
  featuredImgUrl = DEFAULT_FEATURED_IMAGE,
}) => {
  const randomTitle = [
    "Walking On Air ",
    "Amazing Nature",
    ...
    >
      <div className="lg:absolute z-10 lg:left-0 lg:top-1/2 lg:transform lg:-translate-y-1/2 -mt-2 lg:mt-0 sm:px-5 lg:px-0 w-full lg:max-w-lg ">
        <div className="nc-CardLarge1__left p-4 sm:p-8 xl:py-14 md:px-10 bg-white dark:bg-neutral-900 shadow-lg rounded-3xl space-y-3 sm:space-y-8 ">
          ...
          {/* AUTION TIME */}
          <TimeCountDown endDate={new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString()} />
        ...
        <div className="nc-CardLarge1__right ">
          <Link href={"/nft-detail"}>
            <NcImage
              ...
              src={typeof featuredImgUrl === 'string' ? featuredImgUrl : featuredImgUrl?.src || DEFAULT_FEATURED_IMAGE}
              alt={"title"}
          ...
          {/* META FAVORITES */}
          <LikeButton className="absolute right-3 top-3 sm:right-7 sm:top-7" nftId="featured-nft" />
        </div>
      </div>
    </div>
  );
