...
export interface VerifyIconProps {
  ...
  isPlusUser?: boolean;
  isGoldUser?: boolean;
  subscriptionTier?: string; // New prop to handle any tier
}
...
const VerifyIcon: FC<VerifyIconProps> = ({
  ...
  // Determine tier from props - prioritize subscriptionTier prop if available
  const tier = subscriptionTier || (isGoldUser ? 'gold' : (isPlusUser ? 'plus' : null));
  
  if (!tier || tier === 'starter') {
    return null; // Don't show icon for starter/no tier
  }
    ...
    <span className={`${className} flex items-center`}>
      <svg className={iconClass} viewBox="0 0 17 17" fill="none">
        <path
          ...
          fill={isGold ? "#F59E0B" : "#38BDF8"}
          stroke={isGold ? "#F59E0B" : "#38BDF8"}
          strokeWidth="1.5"
          strokeLinecap="round"
      ...
      {(isPlus || isGold) && (
        <span className={`ml-1 text-xs font-medium ${isGold ? 'text-amber-500' : 'text-sky-500'}`}>
          {isGold ? 'gold' : 'plus'}
        </span>
      )}
    </span>
