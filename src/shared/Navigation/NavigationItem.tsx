...
export interface NavigationItemWithRouterProps {
  menuItem: NavItemType;
}

const NavigationItem: FC<NavigationItemWithRouterProps> = ({ menuItem }) => {
  const [menuCurrentHovers, setMenuCurrentHovers] = useState<string[]>([]);
  const [href, setHref] = useState(menuItem.href);
  ...
  useEffect(() => {
    if (menuItem.getHref) {
      menuItem.getHref().then((dynamicHref) => {
        setHref(dynamicHref);
      });
    }
  }, [menuItem]);

  const onMouseEnterMenu = useCallback((id: string) => {
    setMenuCurrentHovers((state) => [...state, id]);
  }, []);

  const onMouseLeaveMenu = useCallback((id: string) => {
        ...
        {() => (
          <>
            ...
            <Transition
              ...
              beforeLeave={() => {
                // Ensure we clean up any stale hover states
                if (menuCurrentHovers.includes(menuDropdown.id)) {
                  onMouseLeaveMenu(menuDropdown.id);
                }
              }}
            >
              <Popover.Panel
        ...
        {() => (
          <>
            ...
            <Transition
              ...
              beforeLeave={() => {
                // Ensure we clean up any stale hover states
                if (menuCurrentHovers.includes(item.id)) {
                  onMouseLeaveMenu(item.id);
                }
              }}
            >
              <Popover.Panel
